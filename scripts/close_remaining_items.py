...
def _write_matrix(full_gate_ok: bool) -> Path:
    ...
    lines = [
        ...
        "| 완료 | n8n/LangGraph 대비 에이전트 확장 (오토힐링/장애자동복구 등) | 완료 |",
        "| 완료 | 시장충격 시뮬레이터, AI 다중교차검증 정책, 한도 자동통제 (v1.2 보완) | 완료 |",
        f"| 완료 | 전체 운영 게이트(outputs/full_operational_gate_report.json) | {'완료' if full_gate_ok else '미완료'} |",
        "| 예외(물리/현실) | 실제 법무/세무 전문가의 대면 자문·서명(법적 효력) | 예외/미완료 |",
        "| 예외(물리/현실) | 실키 기반 장기 실거래 수익 검증(월단위) | 예외/미완료 |",
    ]
    p.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return p
